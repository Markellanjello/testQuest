...
def main():
    ...
    def search_dir():
        ...
        query = cur.execute("SELECT * FROM directory WHERE " + " OR ".join([f"{col} LIKE '%{item}%'" for col in ['id', 'surname', 'name', 'father_name', 'org_name', 'phone_pub', 'work_phone'] for item in search_items])).fetchall()
        pd_show(query)
    #Функция добавления
    def directory_dump():
        person = input(
